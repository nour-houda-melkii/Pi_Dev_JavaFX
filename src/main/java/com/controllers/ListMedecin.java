...
public class ListMedecin implements Initializable {
    ...
    private void handleAddMedecin() {
        try {
            ...
            // Optionnel: définir le propriétaire de la nouvelle fenêtre (la fenêtre principale)
            stage.initOwner(addButton.getScene().getWindow());

            // Empêcher la fenêtre principale d'être utilisée pendant que la nouvelle est ouverte
            stage.initModality(Modality.WINDOW_MODAL);

            // Obtenir le contrôleur et définir le callback
            AjouterMedecin controller = loader.getController();
            controller.setOnMedecinAddedCallback(() -> {
                refreshMedecinList();  // Méthode pour rafraîchir la liste
            });
    ...
    private void refreshMedecinList() {
        List<User> medecins = userService.rechercherTousMedecins();  // Récupère la liste mise à jour depuis le service
        ObservableList<User> observableMedecins = FXCollections.observableArrayList(medecins);
        medecinListView.setItems(observableMedecins);  // Met à jour la ListView
    }

    private void refreshMedecinUnverified() {
        List<User> medecins = userService.rechercherMedecinsNonVerifies();  // Récupère la liste mise à jour depuis le service
        ObservableList<User> observableMedecins = FXCollections.observableArrayList(medecins);
        medecinListView.setItems(observableMedecins);  // Met à jour la ListView
    }
    ...
    private void handleEditMedecin(User medecin) {
        try {
            ...
            controller.setOnMedecinUpdatedCallback(() -> {
                refreshMedecinList(); // Callback pour rafraîchir la liste
            });
    ...
    private void handleShowUnverified() {
        try {
            ...
            controller.setOnVerificationCallback(() -> {
                refreshMedecinList(); // Rafraîchir la liste principale
            });
            controller.setOnDeletionCallback(() -> {
                refreshMedecinList(); // Rafraîchir après suppression
            });
