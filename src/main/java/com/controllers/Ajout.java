...
onlineAppointmentsPane.setVisible(false);
    ...
    private void displayOnlineAppointmentsInGrid(List<RendezVous> appointments) {
        ...
        onlineAppointmentsGrid.getChildren().clear();
        onlineAppointmentsGrid.getRowConstraints().clear();
        onlineAppointmentsGrid.getColumnConstraints().clear();

        // Définir les en-têtes de colonnes
        String[] headers = {"Date", "Heure", "Médecin", "État", "Lien", "Actions"};
        for (int i = 0; i < headers.length; i++) {
            Label headerLabel = new Label(headers[i]);
            headerLabel.setStyle("-fx-font-weight: bold; -fx-padding: 5px;");
            onlineAppointmentsGrid.add(headerLabel, i, 0);
        }
        ...
        for (int i = 0; i < appointments.size(); i++) {
            ...
            // État du rendez-vous (vous devrez peut-être adapter selon votre modèle)
            String etatStr = "En ligne";  // Puisque ce sont tous des rendez-vous en ligne

            // Ajouter les informations dans la grille
            onlineAppointmentsGrid.add(new Label(dateStr), 0, i + 1);
            onlineAppointmentsGrid.add(new Label(heureStr), 1, i + 1);
            onlineAppointmentsGrid.add(new Label(medecinName), 2, i + 1);
            onlineAppointmentsGrid.add(new Label(etatStr), 3, i + 1);

            // Afficher le lien Jitsi (ou une indication s'il n'y en a pas)
            String lienJitsi = rdv.getLienJitsi();
            Label lienLabel = new Label(lienJitsi != null && !lienJitsi.isEmpty() ?
                    "Disponible" : "Non disponible");
            onlineAppointmentsGrid.add(lienLabel, 4, i + 1);

            // Créer un bouton pour sélectionner ce rendez-vous
            Button selectButton = new Button("Sélectionner");
            ...
            selectButton.setOnAction(event -> {
                ...
                boolean hasValidLink = selectedRdv.getLienJitsi() != null && !selectedRdv.getLienJitsi().isEmpty();
            ...
            // Ajouter le bouton dans une HBox pour un meilleur alignement
            HBox actionBox = new HBox(10);
            actionBox.setAlignment(Pos.CENTER);
            actionBox.getChildren().add(selectButton);
            onlineAppointmentsGrid.add(actionBox, 5, i + 1);
        }
