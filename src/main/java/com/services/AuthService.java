...
////////////////////////////////////////////////////////////
    ...
    private void incrementLoginAttempts(String email) throws AuthException {
        // D'abord vérifier si le compte est déjà bloqué
        checkIfAccountIsLocked(email);

        String query = "UPDATE user SET failed_login_attempts = failed_login_attempts + 1 WHERE email = ? AND is_blocked = FALSE";

        try (PreparedStatement pst = connection.prepareStatement(query)) {
            pst.setString(1, email);
            int updated = pst.executeUpdate();

            if (updated > 0) { // seulement si la mise à jour a réussi
    ...
    private void checkAndLockAccountIfNeeded(String email) throws AuthException {
        String query = "SELECT failed_login_attempts, is_blocked FROM user WHERE email = ?";
        ...
        try (PreparedStatement pst = connection.prepareStatement(query)) {
            ...
            try (ResultSet rs = pst.executeQuery()) {
                if (rs.next()) {
                    ...
                    if (!isBlocked && attempts >= MAX_LOGIN_ATTEMPTS) {
                        lockAccount(email); // Bloque le compte
                        sendLockNotificationEmail(email); // Envoie un email
                    }
                }
            }
        } catch (SQLException e) {
            throw new AuthException("Erreur lors de la vérification des tentatives");
        }
    }
    ...
    private void lockAccount(String email) throws SQLException {
        ...
        String query = "UPDATE user SET is_blocked = TRUE, blocked_until = ?, failed_login_attempts = 0 WHERE email = ?";
