...
public class EventController {
    ...
    @FXML
    private ScrollPane archivedEventsScrollPane;
    ...
    public void initialize() {
        // Configure active events container
        if (eventsScrollPane != null) {
            // Bind width and set proper properties to fill the space
            eventsContainer.prefWidthProperty().bind(eventsScrollPane.widthProperty().subtract(20));
            eventsScrollPane.setFitToWidth(true);
            eventsContainer.setAlignment(Pos.CENTER);
            
            // Add a listener to adjust wrap length when window resizes
            eventsScrollPane.widthProperty().addListener((obs, oldVal, newVal) -> {
                // Set the wrap length to a multiple of card width (320px) plus gaps
                int cardsPerRow = Math.max(1, (int)((double)newVal.intValue() / 345));
                double wrapLength = cardsPerRow * 345;
                eventsContainer.setPrefWrapLength(wrapLength);
            });
        }
        
        // Configure archived events container
        ScrollPane archiveScrollPane = (ScrollPane) archivedEventsContainer.getParent();
        if (archiveScrollPane != null) {
            archivedEventsContainer.prefWidthProperty().bind(archiveScrollPane.widthProperty().subtract(20));
            archiveScrollPane.setFitToWidth(true);
            archivedEventsContainer.setAlignment(Pos.CENTER);
            
            // Add a listener to adjust wrap length when window resizes
            archiveScrollPane.widthProperty().addListener((obs, oldVal, newVal) -> {
                // Set the wrap length to a multiple of card width (320px) plus gaps
                int cardsPerRow = Math.max(1, (int)((double)newVal.intValue() / 345));
                double wrapLength = cardsPerRow * 345;
                archivedEventsContainer.setPrefWrapLength(wrapLength);
            });
        }
    ...
    private void handleAddEvent(ActionEvent event) {
        try {
            Parent formPage = FXMLLoader.load(getClass().getResource("/com/views/event/event_form.fxml"));
    ...
    private void handleEditEvent(Event event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/com/views/event/event_form.fxml"));
